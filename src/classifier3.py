...
# import seaborn as sns # Optional, for a pretty heatmap
...
#     """Chops the image into a sequence of small squares (patches)."""
...
#     """Turns patches into vectors and adds position information."""
...
#     if model_type == "cnn" or model_type == "vit":
...
#         num_heads = 4
#         transformer_layers = 4# Increased slightly for better depth
...
#         # FIX: Chain the layers correctly
...
#             x3 = layers.Dense(projection_dim * 4, activation=tf.nn.gelu)(x3) # Ratio 4
#             x3 = layers.Dropout(0.1)(x3)
#             x3 = layers.Dense(projection_dim, activation=tf.nn.gelu)(x3)
#             encoded_patches = layers.Add()([x3, x2])

#         # Use GAP instead of Flatten for better stability
...
#         start = time.time()
#         # It's also helpful to use a slightly larger batch size for ViTs if your memory allows
...
#     # elif args.mode == "test":
#     #     model = tf.keras.models.load_model(model_path, custom_objects={"PatchExtract": PatchExtract, "PatchEmbedding": PatchEmbedding})
#     #     loss, accuracy = model.evaluate(X_test, y_test, verbose=0)
#     #     print(f"Test Accuracy: {accuracy:.4f}")

#     #     startp = time.time()
#     #     y_pred = model.predict(X_test, verbose=0).argmax(axis=1)
#     #     print(f"Prediction Duration: {time.time() - startp:.2f}s")
...
#         # 2. Print the Professional Report
...
#         # 4. Save the Confusion Matrix as a Plot
#         plt.figure(figsize=(12, 10))
#         sns.heatmap(cm, annot=True, fmt='d', xticklabels=class_names, yticklabels=class_names, cmap='Blues')
#         plt.xlabel('Predicted')
#         plt.ylabel('True')
#         plt.title(f'Confusion Matrix: {args.model_type} on {args.dataset}')
        
#         cm_path = f"plots/{args.model_type}_{args.dataset}_cm.png"
#         plt.savefig(cm_path)
#         plt.close()
...
def main():
    ...
    if args.mode == "test":
        # Load model with custom objects (now properly configured)
